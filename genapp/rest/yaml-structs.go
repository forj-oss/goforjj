...
// __MYPLUGIN: {{     range $FlagName, $Flag := $Group.Flags }}\
	data1 string `json:"data-data1"` // __MYPLUGIN: 	{{ go_vars $FlagName }} {{if $Flag.List}}[]{{end}}string `json:"{{ $GroupName }}-{{ $FlagName }}"`
// __MYPLUGIN: {{     end }}\
}
...
// __MYPLUGIN: {{   range $ParamName, $Opts := $Object.Flags }}\
	Param1 string // __MYPLUGIN: 	{{ go_vars $ParamName }} {{if $Opts.List}}[]{{end}}string `json:"{{ $ParamName }}"`// {{ $Opts.Help }}
// __MYPLUGIN: {{   end }}\
